...
载荷长度或 ISO 描述符数量的截断上限。ISO 帧可以复用重叠的缓冲区区域；
重组时会检查重复字节是否一致，并按每一帧核对复制字节数。非 ISO 的重叠传输片段，
以及内容冲突的 ISO 重叠区域会被拒绝。完整性和格式限制见
[抓包语义](usage.zh-CN.md#抓包语义)与[使用限制](usage.zh-CN.md#当前使用限制)。
原始归档回放还会按传输阶段、请求/实际长度和 ISO 描述符核对声明的载荷范围；
归档不能通过缩小载荷长度或清除数据标记，将缺少字节的事件伪装为完整事件。
