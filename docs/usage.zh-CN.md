...
自动化采集可以使用 `--ready-file PATH`，在全部探针挂载成功后写入就绪标记；该选项只适用于
实时抓包。创建输出前，程序会检查输入、输出与就绪标记是否指向同一个文件，包含软链接别名
以及第一个轮转文件，避免相互覆盖。
...
临时存储进行流式处理。因抓包截断长度或原采集器无法读取缓冲区而缺失的数据，
都会被报告为不完整数据。
标准 USB pcapng 不包含 VID/PID；完成事件的原始请求长度和延迟也需要观测到对应的提交事件
才能确定。缺失字段在过滤器中保留为未知值。ISO 完成事件即使缺少提交事件也能读取，
包括带空隙的帧和末尾的空帧；实际传输字节数不会被当作原始缓冲区的长度。
请求配对和 ISO 统计按输入接口及 section 区分来源。重写文件时，各来源仍使用不同接口，
即使它们的 URB ID 或 USB 总线、设备地址相同，也不会被合并。
